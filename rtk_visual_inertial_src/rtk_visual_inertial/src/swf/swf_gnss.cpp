...
void SWFOptimization::GnssProcess(mea_t* rover) {
    ...
    // 执行传统瞬时RAIM检测
    if(rover_count > 0) {
        last_raim_result_ = PerformTraditionalRaim(*rovers[rover_count - 1]);
        
        // 输出RAIM结果
        LogRaimResult(last_raim_result_);
        
        // 根据RAIM结果调整处理策略
        if(last_raim_result_.integrity_available) {
            if(last_raim_result_.fault_detected) {
                // 检测到故障，降低GNSS权重
                printf("GNSS RAIM: Fault detected! Satellite PRN=%d, Test=%.3f, Threshold=%.3f\n", 
                       last_raim_result_.faulty_satellite_prn,
                       last_raim_result_.test_statistic,
                       last_raim_result_.chi2_threshold);
                
                // 根据故障调整权重
                AdjustGnssWeightsForFault(last_raim_result_);
            } else {
                printf("GNSS RAIM: Integrity OK, HPL=%.2fm, VPL=%.2fm, GDOP=%.2f\n",
                       last_raim_result_.protection_level_horizontal,
                       last_raim_result_.protection_level_vertical,
                       last_raim_result_.gdop);
            }
        } else {
            printf("GNSS RAIM: Integrity not available, Satellites=%d\n", 
                   last_raim_result_.available_satellites);
        }
    }
...
#define CONDITION problem.HasParameterBlock(pointer)&&!problem.IsParameterBlockConstant(pointer)&&mark_add.find(pointer)==mark_add.end()
    ...
    // 移除重复的RAIM检测代码 - RAIM已在GnssProcess函数中执行
