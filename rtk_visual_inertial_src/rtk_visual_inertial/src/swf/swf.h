...
class SWFOptimization {
  public:
    ...
    struct ResidualStats {
        ...
        double rms_error = 0.0;         // RMS误差  
        double weighted_norm = 0.0;     // 加权残差范数
    };

    // 当前残差信息结构体
    struct CurrentResiduals {
        double timestamp = 0.0;
        Eigen::Vector3d position = Eigen::Vector3d::Zero();
        
        // 各类残差数据
        std::vector<double> gnss_residuals;           // 原始GNSS残差
        std::vector<double> gnss_weighted_residuals;  // 加权GNSS残差
        Eigen::Matrix<double, 15, 1> imu_residual = Eigen::Matrix<double, 15, 1>::Zero();           // 原始IMU残差
        Eigen::Matrix<double, 15, 1> imu_weighted_residual = Eigen::Matrix<double, 15, 1>::Zero(); // 加权IMU残差
        std::vector<std::pair<int, Eigen::Vector2d>> visual_residuals;           // 原始视觉残差
        std::vector<std::pair<int, Eigen::Vector2d>> visual_weighted_residuals;  // 加权视觉残差
        Eigen::VectorXd prior_residual;
        
        // 统计信息
        ResidualStats gnss_stats;
        ResidualStats imu_stats; 
        ResidualStats visual_stats;
        ResidualStats prior_stats;
        
        double total_residual_norm = 0.0;
        double optimization_cost = 0.0;  // Ceres求解器的最终代价函数值
        int imu_factor_type = 0; // 0: 标准IMU, 1: IMU-GNSS联合
    };

    // 当前残差信息
    CurrentResiduals current_residuals;

    // RAIM检测结果结构体
    struct TraditionalRaimResult {
        bool integrity_available = false;           // 完好性是否可用
        bool fault_detected = false;               // 是否检测到故障
        double test_statistic = 0.0;               // 检验统计量
        double chi2_threshold = 0.0;               // χ²阈值
        double protection_level_horizontal = 0.0;   // 水平保护级
        double protection_level_vertical = 0.0;     // 垂直保护级
        int faulty_satellite_prn = -1;             // 故障卫星PRN
        double gdop = 0.0;                         // 几何精度因子
        int available_satellites = 0;              // 可用卫星数
        double detection_probability = 0.0;        // 检测概率
        double false_alarm_rate = 0.0;             // 虚警率
        std::string failure_reason = "";           // 故障原因描述
    };

    // RAIM配置参数结构体
    struct RaimConfig {
        double false_alarm_rate = 0.001;           // 虚警率
        double missed_detection_rate = 0.001;      // 漏检率
        double min_elevation_angle = 10.0;         // 最小高度角(度)
        int min_satellites = 5;                    // 最小卫星数
        double max_gdop = 6.0;                     // 最大GDOP
        bool enable_fault_isolation = true;        // 启用故障隔离
    };

    // RAIM配置和状态
    RaimConfig raim_config_;
    TraditionalRaimResult last_raim_result_;

    // 残差评估函数
    void EvaluateGnssResiduals();
    void EvaluateImuResiduals(); 
    void EvaluateVisualResiduals();
    void EvaluatePriorResiduals();
    void EvaluateAllResiduals();
    ResidualStats CalculateGnssStats(const std::vector<double>& gnss_residuals);
    ResidualStats CalculateImuStats(const Eigen::Matrix<double, 15, 1>& imu_residual);
    ResidualStats CalculateVisualStats(const std::vector<std::pair<int, Eigen::Vector2d>>& visual_residuals);
    ResidualStats CalculatePriorStats(const Eigen::VectorXd& prior_residual);
    void CalculateAllStats();

    // RAIM核心函数声明
    TraditionalRaimResult PerformTraditionalRaim(const mea_t& gnss_data);
    bool BuildGeometryMatrix(const mea_t& gnss_data, 
                            Eigen::MatrixXd& H, 
                            Eigen::VectorXd& y, 
                            Eigen::MatrixXd& W);
    Eigen::Vector3d EstimateUserPosition(const mea_t& gnss_data);
    Eigen::MatrixXd CalculateParityMatrix(const Eigen::MatrixXd& H, 
                                         const Eigen::MatrixXd& W);
    int IsolateFaultySatellite(const mea_t& gnss_data);
    double CalculateProtectionLevel(const Eigen::MatrixXd& H, 
                                   const Eigen::MatrixXd& W, 
                                   double threshold,
                                   bool horizontal = true);
    double CalculateChi2Threshold(int dof, double false_alarm_rate);
    void InitializeRaimConfig();
    void LogRaimResult(const TraditionalRaimResult& result);
    void AdjustGnssWeightsForFault(const TraditionalRaimResult& raim_result);
